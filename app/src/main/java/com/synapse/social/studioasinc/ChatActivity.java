...
public class ChatActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		imageview2.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), "Coming up!");
			}
		});
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (_charSeq.length() == 0) {
					if (img_container_layout.getVisibility() == View.VISIBLE && (file_type_expand == 1)) {
						...
						/*
camera_gallery_btn_container_round.setVisibility(View.VISIBLE);
*/
...
*/
	...
	private void initializeLogic() {
		...
		_getUserReference();
		/*  This entire block is commented out to disable the fade-on-scroll effect.
    It prevents the app from running calculations every time the user scrolls.
...
*/
	...
	public void _messageOverviewPopup(final View _view, final double _position, final ArrayList<HashMap<String, Object>> _data) {
		...
		// Reaction buttons
		final TextView thumbReaction = pop1V.findViewById(R.id.thumb_reaction_btn);
		final TextView loveReaction = pop1V.findViewById(R.id.Love_reaction_btn);
		final TextView wowReaction = pop1V.findViewById(R.id.Cat_Wow_reaction_btn);
		final TextView laughReaction = pop1V.findViewById(R.id.Cat_Laught_reaction_btn);
		final TextView sadReaction = pop1V.findViewById(R.id.Cat_Sad_reaction_btn);
		final TextView dislikeReaction = pop1V.findViewById(R.id.Dislike_reaction_btn);
		final TextView coolReaction = pop1V.findViewById(R.id.Cool__reaction_btn);
		...
		thumbReaction.setOnClickListener(new View.OnClickListener() {
			    @Override
			    public void onClick(View v) {
				        vbr.vibrate(48);
				        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();
				        String messageKey = _data.get((int)_position).get("key").toString();
				        String otherUserId = getIntent().getStringExtra("uid");
				        
				        // Check if user already reacted with this emoji
				        if (_data.get((int)_position).containsKey("reactions")) {
					            Map<String, Object> existingReactions = (Map<String, Object>) _data.get((int)_position).get("reactions");
					            if (existingReactions.containsKey(currentUserId)) {
						                // If same emoji, remove reaction
						                if (existingReactions.get(currentUserId).equals("👍")) {
							                    // Remove reaction
							                    Map<String, Object> reactionUpdate = new HashMap<>();
							                    reactionUpdate.put("reactions/" + currentUserId, null);
							                    
							                    FirebaseDatabase.getInstance().getReference("skyline/chats")
							                        .child(currentUserId)
							                        .child(otherUserId)
							                        .child(messageKey)
							                        .updateChildren(reactionUpdate);
				        ...
				        // Add new reaction
				        Map<String, Object> reactionUpdate = new HashMap<>();
				        reactionUpdate.put("reactions/" + currentUserId, "👍");
		...
		loveReaction.setOnClickListener(new View.OnClickListener() {
			    @Override
			    public void onClick(View v) {
				        vbr.vibrate(48);
				        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();
				        ...
				        if (_data.get((int)_position).containsKey("reactions")) {
					            ...
					            if (existingReactions.containsKey(currentUserId)) {
						                // If same emoji, remove reaction
						                if (existingReactions.get(currentUserId).equals("❤️")) {
							                    // Remove reaction
							                    Map<String, Object> reactionUpdate = new HashMap<>();
							                    reactionUpdate.put("reactions/" + currentUserId, null);
				        ...
				        // Add new reaction
				        Map<String, Object> reactionUpdate = new HashMap<>();
				        reactionUpdate.put("reactions/" + currentUserId, "❤️");
		...
		wowReaction.setOnClickListener(new View.OnClickListener() {
			    @Override
			    public void onClick(View v) {
				        vbr.vibrate(48);
				        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();
		...
		laughReaction.setOnClickListener(new View.OnClickListener() {
			    @Override
			    public void onClick(View v) {
				        vbr.vibrate(48);
				        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();
		...
		dislikeReaction.setOnClickListener(new View.OnClickListener() {
			    @Override
			    public void onClick(View v) {
				        vbr.vibrate(48);
				        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();
		...
		coolReaction.setOnClickListener(new View.OnClickListener() {
			    @Override
			    public void onClick(View v) {
				        vbr.vibrate(48);
				        String currentUserId = FirebaseAuth.getInstance().getCurrentUser().getUid();
		...
		edit.setOnClickListener(new View.OnClickListener() {
			    ...
			    public void onClick(View _view) {
				        ...
				        cd.setView(cdCV);
				        final LinearLayout add_button = cdCV.findViewById(R.id.add_button);
				        final EditText image_url_input = cdCV.findViewById(R.id.image_url_input);
				        final TextInputLayout image_url_input_inputlayout = cdCV.findViewById(R.id.image_url_input_inputlayout);
				        final TextView dialog_top_title = cdCV.findViewById(R.id.dialog_top_title);
				        final TextView subtitle = cdCV.findViewById(R.id.subtitle);
				        final TextView textview3 = cdCV.findViewById(R.id.textview3);
				        ...
				        image_url_input.setMaxLines(8);
				        image_url_input_inputlayout.setBoxStrokeColor(0xFF494C54);
				        image_url_input_inputlayout.setBoxBackgroundMode(TextInputLayout.BOX_BACKGROUND_OUTLINE);
				        image_url_input_inputlayout.setBoxCornerRadii(50, 50, 50, 50);
				        add_button.setOnClickListener(new View.OnClickListener() {
					            ...
					            public void onClick(View _view) {
						                if (!image_url_input.getText().toString().isEmpty()) {
							                    ...
							                    ChatSendMap.put("push_date", String.valueOf(cc.getTimeInMillis()));
							                    
							                    FirebaseDatabase.getInstance().getReference("skyline/chats")
							                        .child(getIntent().getStringExtra("uid"))
							                        .child(FirebaseAuth.getInstance().getCurrentUser().getUid())
							                        .child(_data.get((int)_position).get("key").toString())
...
*/
		...
		zorry.setPositiveButton("Delete", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface _dialog, int _which) {
				if (_data.get((int)_position).containsKey("message_image_uri")) {
					if (_data.get((int)_position).get("message_image_uri").toString().equals("")) {
						
					} else {
						_firebase_storage.getReferenceFromUrl(_data.get((int)_position).get("message_image_uri").toString()).delete().addOnSuccessListener(_upload_selected_img_delete_success_listener).addOnFailureListener(_upload_selected_img_failure_listener);
						_LoadingDialog(true);
					}
				} else {
	...
	public void _send_btn() {
		if (file.equals("")) {
			...
			upload_selected_img.child(filename).putFile(Uri.fromFile(new File(file))).addOnFailureListener(_upload_selected_img_failure_listener).addOnProgressListener(_upload_selected_img_upload_progress_listener).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
				@Override
				public Task<Uri> then(Task<UploadTask.TaskSnapshot> task) throws Exception {
					return upload_selected_img.child(filename).getDownloadUrl();
				}}).addOnCompleteListener(_upload_selected_img_upload_success_listener);
		}
	}
	...
	public void _UI() {
		...
		//_ImageColor(mMessageReplyLayoutBodyIc, 0xFF445E91);
	...
	public class ChatMessagesListRecyclerAdapter extends RecyclerView.Adapter<ChatMessagesListRecyclerAdapter.ViewHolder> {
		...
		public void onBindViewHolder(ViewHolder _holder, final int _position) {
			...
			final TextView reaction = _view.findViewById(R.id.reaction);
...
*/
			...
			if (_data.get((int)_position).containsKey("typingMessageStatus")) {
				    ...
				    _textview_mh(message_text, _data.get((int)_position).get("message_text").toString());
				    // FIXED: Removed extra parenthesis in the following line
				    push.setTimeInMillis((long)Double.parseDouble(_data.get((int)_position).get("push_date").toString()));
				    date.setText(new SimpleDateFormat("hh:mm a").format(push.getTime()));
				    if (_data.get((int)_position).containsKey("replied_message_id")) {
					        {
					        ...
					        // Add click listener to scroll to replied message
					        final String repliedMsgId = _data.get((int)_position).get("replied_message_id").toString();
					        mRepliedMessageLayout.setTag(repliedMsgId);
					        mRepliedMessageLayout.setOnClickListener(new View.OnClickListener() {
						            @Override
						            public void onClick(View v) {
							                String targetMsgId = (String) v.getTag();
							                int targetPos = -1;
							                for (int i = 0; i < _data.size(); i++) {
								                    if (_data.get(i).containsKey("key") && _data.get(i).get("key").toString().equals(targetMsgId)) {
									                        targetPos = i;
									                        break;
									                    }
								                }
							                if (targetPos != -1) {
								                    RecyclerView recyclerView = (RecyclerView) _view.getParent();
